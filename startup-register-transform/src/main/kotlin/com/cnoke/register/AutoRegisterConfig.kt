...
//                project.logger.error("auto register config error: scanInterface, codeInsertToClassName and registerMethodName should not be null\n$info")
    ...
    fun convertConfig(registerInfo: List<Map<String, Any>>) {
        registerInfo.forEach { map ->
            ...
            map[INTERFACE_NAME]?.let {
                info.interfaceName = it as String
            }
            ...
            info.superClassNames = superClasses as ArrayList<String>
            map[INSERT_TO_CLASS_NAME]?.let {
                info.initClassName = it as String //代码注入的类
            }
            map[INSERT_TO_METHOD_NAME]?.let {
                info.initMethodName = it as String //代码注入的方法（默认为static块）
            }
            map[REGISTER_METHOD_NAME]?.let {
                info.registerMethodName = it as String //生成的代码所调用的方法
            }
            map[REGISTER_CLASS_NAME]?.let {
                info.registerClassName = it as String //注册方法所在的类
            }
            map["include"]?.let {
                info.include = it as ArrayList<String>
            }
            map["exclude"]?.let {
                info.exclude = it as ArrayList<String>
            }
            map[IS_INSTANCE]?.let {
                info.isInstance = it as Boolean
            }
    ...
    fun reset() {
        ...
        RegisterInfo.clearFileContainsInitClassMap()
    }
