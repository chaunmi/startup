...
public class RegisterPlugin implements Plugin<Project> {
    ...
    static void init(Project project, RegisterTransform transformImpl) {
        ...
        println "AutoRegisterConfig init size: " + config.registerInfo.size()
        if(config.registerInfo.size() > 0) {
            config.registerInfo.forEach {
                it.entrySet().forEach {
                    println  " plugin init " + it.key + ": " + it.value
                }
            }
        }
        println "AutoRegisterConfig init end "
