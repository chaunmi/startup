...
object AutoRegisterHelper {
    ...
    fun getRegisterCacheJarFile(project: Project, name: String): File {
        val baseDir = getCacheFileDir(project)
        if(mkdirs(baseDir)) {
     ...
     * @return
     */
    fun readToMap(file: File, type: Type): HashMap<String, ScanJarHarvest> {
        var map: HashMap<String, ScanJarHarvest>? = null
        if (file.exists()) {
            if (type != null) {
